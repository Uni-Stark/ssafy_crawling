...
def preprocess_talks(src):
    ...
    with open(src) as file:
        ...
        for talk in reader:
            try:
                talk = {
                    'title': talk[14],     # 강연의 제목
                    'speaker': talk[6],   # 강연자의 이름
                    'views': int(talk[16]),     # 조회수
                    'comments': int(talk[0]),  # 댓글의 개수
                    'duration': int(talk[2]),  # 강연 길이
                    'languages': int(talk[5]), # 지원하는 언어의 수
                    'tags': talk[13],      # 관련 태그 (키워드)
                    'ratings': jsonify(talk[10]),   # 강의에 대한 평가
                }
            except:
                pass
            talks.append(talk)
...
def show_ratings(talk):
    ...
    '''
    pass
    keywords = [keys['keywords'] for keys in talk]
    counts = [cnt['ratings'] for cnt in talk]
